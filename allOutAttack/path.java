...
//This is currently a copy of moveTo1's RobotPlayer.java
import java.util.*;
...
public class path {
	...
	public static void move1(boolean mFlag, MapLocation self, MapLocation targ) throws GameActionException 
	{
		int selfX = self.x;
		int selfY = self.y;
		int destX = targ.x;
		...
		double dx = ((double)destX) - ((double)selfX);
		double dy = ((double)destY) - ((double)selfY);
	...
	public static void seekSimple(MapLocation selfLoc, MapLocation target) throws GameActionException {
		if(rc.isActive())
		{
			move1(true, selfLoc, target);
		}
	}
